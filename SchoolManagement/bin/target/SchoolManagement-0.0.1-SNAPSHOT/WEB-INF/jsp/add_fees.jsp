					...
					<div id="remove-scroll" class="left-sidemenu">
						<ul class="sidemenu  page-header-fixed slimscroll-style"
							...
							<li class="nav-item active open"><a href="#"
								class="nav-link nav-toggle"> <i class="material-icons">monetization_on</i>
									<span class="title">Fees</span> <span class="selected"></span>
									<span class="arrow open"></span>
							</a>
								<ul class="sub-menu">
									
									<li class="nav-item active"><a href="add_fees"
										class="nav-link "> <span class="title">Add Fees </span> <span
											class="selected"></span>
									</a></li>
				...
				<div class="page-content">
					<div class="page-bar">
						<div class="page-title-breadcrumb">
							...
							<ol class="breadcrumb page-breadcrumb pull-right">
								...
								<li><a class="parent-item" href="">Fees</a>&nbsp;<i
									...
									<form action="/add_fee" method="post">
										...
										<div class="form-row">
											<div class="form-group col-md-4">
												<label>Student Department</label> <select
													class="custom-select mr-sm-2" name="std"
													required="required" id="list2">
													<option value="" selected>Choose...</option>
													<c:forEach var="stdname" items="${StdName}">
														<option value="${stdname.id}">${stdname.stdName}</option>
													</c:forEach>
												</select>
											</div>
											<div class="form-group col-md-4">
												<label>Student Div</label> <select
											...
											<div class="form-group col-md-4">
												<label>Student RollNo</label> <select
													class="custom-select mr-sm-2" name="stu"
										...
										<div class="form-row">
											...
											<div class="form-group col-md-6">
												<label for="inputCity">Collection Date</label> <input
													class="form-control" name="collectionDate" type="text"
													id="dateOfBirth" placeholder="Collection Date">
											</div>
											
										</div>
										<div class="form-row">
											<div class="form-group col-md-12">
												<label for="inputCity">Fees Details</label>
												<textarea name="Details" placeholder="Fees Details" class="form-control" rows="4" id="text7"></textarea> 
											</div>
										</div>
										<br>
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="card-box">
								<div class="card-head">
									<header>Fee Recpit</header>

								</div>
								<div class="card-body" id="bar-parent2">
									<form action="/recpit" method="post">

										<div class="form-row">
											<div class="form-group col-md-4">
												<label>Student Department</label> <select
													class="custom-select mr-sm-2" name="std"
													required="required" id="list3">
													<option value="" selected>Choose...</option>
													<c:forEach var="stdname" items="${StdName}">
														<option value="${stdname.id}">${stdname.stdName}</option>
													</c:forEach>
											...
											<div class="form-group col-md-4">
												<label>Student Div</label> <select
													class="custom-select mr-sm-2" name="div"
													required="required" id="divElement1">
													<option value="" selected>Choose...</option>
												</select>
											</div>
											...
											<div class="form-group col-md-4">
												<label>Student RollNo</label> <select
													class="custom-select mr-sm-2" name="stu"
													required="required" id="studentName1">
													<option value="" selected>Choose...</option>
												</select>
											</div>
										</div>
	...
	<script>
		$("#divElement").change(
				function() {
					...
					$('#studentName').find('option').remove();
					$.ajax({
						url : "http://quickedu.co.in/findByStdAndDiv?std="
								+ $("#list2").val() + "&div=" + $(this).val(),
						success : function(result) {
							for (var i = 0; i < result.length; i++) {
								var node = document.createElement("option");
								node.setAttribute("value", result[i].id);
								var textnode = document
										.createTextNode(result[i].rollNo + " "
												+ result[i].name + " "
												+ result[i].lastName);
		...
		function abc() {
			var myOpts = document.getElementById('divElement').options;
			$.ajax({
				url : "http://quickedu.co.in/findByStdAndDiv?std="
						+ $("#list2").val() + "&div=" + myOpts[0].value,
				success : function(result) {
					for (var i = 0; i < result.length; i++) {
						var node = document.createElement("option");
						node.setAttribute("value", result[i].id);
						var textnode = document.createTextNode(result[i].rollNo
								+ " " + result[i].name + " "
								+ result[i].lastName);
						node.appendChild(textnode);
						componentHandler.upgradeElement(node);
						var list = document.getElementById("studentName");
						list.appendChild(node);
						componentHandler.upgradeDom();
					}
				}
		...
		$("#list2").change(function() {
			...
			$.ajax({
				url : "http://quickedu.co.in/findByStdID?std=" + $(this).val(),
				success : function(result) {
					for (var i = 0; i < result.length; i++) {
						var node = document.createElement("option");
						node.setAttribute("value", result[i].id);
	...
	<script>
		$("#divElement1").change(
				function() {
					...
					$('#studentName1').find('option').remove();
					$.ajax({
						url : "http://quickedu.co.in/findByStdAndDiv?std="
								+ $("#list3").val() + "&div=" + $(this).val(),
						success : function(result) {
							for (var i = 0; i < result.length; i++) {
								var node = document.createElement("option");
								node.setAttribute("value", result[i].id);
		...
		function abc1() {
			var myOpts = document.getElementById('divElement1').options;
			$.ajax({
				url : "http://quickedu.co.in/findByStdAndDiv?std="
						+ $("#list3").val() + "&div=" + myOpts[0].value,
				success : function(result) {
					for (var i = 0; i < result.length; i++) {
						var node = document.createElement("option");
						node.setAttribute("value", result[i].id);
						...
						node.appendChild(textnode);
						componentHandler.upgradeElement(node);
						var list = document.getElementById("studentName1");
						list.appendChild(node);
						componentHandler.upgradeDom();
					}
				}
		...
		$("#list3").change(function() {
			$('#divElement1').find('option').remove();
			$.ajax({
				url : "http://quickedu.co.in/findByStdID?std=" + $(this).val(),
				success : function(result) {
					for (var i = 0; i < result.length; i++) {
						...
						node.appendChild(textnode);
						componentHandler.upgradeElement(node);
						var list = document.getElementById("divElement1");
						list.appendChild(node);
						componentHandler.upgradeDom();
					}
					abc1();
